...
# Lunge dataset landmark columns used by trained model
LANDMARK_NAMES = [
    "nose",
    "left_shoulder",
    "right_shoulder",
...
class LungePosturePredictor:
    """
    ML predictor for lunge posture classification.
    Uses trained Random Forest model: lunge_model.pkl
    ...
    def load_model(self):
        if not os.path.exists(self.model_path):
            raise FileNotFoundError(f"Lunge model not found: {self.model_path}")
        ...
        print("Lunge posture prediction model loaded successfully.")
        print("Model path:", self.model_path)

        if self.metrics:
            print("Saved lunge model metrics:", self.metrics)

    def extract_features(self, landmarks):
        """
        Extract the exact landmark coordinate features used during training.
        Columns must match err.train.csv / err.test.csv:
        ...
        for name in LANDMARK_NAMES:
            ...
            # Training dataset has *_v columns, so include it safely.
